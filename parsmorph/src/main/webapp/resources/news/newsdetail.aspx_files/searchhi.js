...
function CustomSearchHighlight1(strSearchKeyWords)
{
	words = strSearchKeyWords.split(" "); 
...
function CustomSearchHighlight(strSearchKeyWords)
{
	words = strSearchKeyWords.split("%20"); 
	
	for (w=0;w<words.length;w++) {
		
		if( words[w] == '' ) continue;
		if(document.getElementById("dgSearch")){
			
			highlightWord(document.getElementById("dgSearch"),words[w]);
			//highlightWord(document.getElementById("dgSearch"),'��?�? �� �����? ����');
			searchhi_string = searchhi_string + ' "' + words[w] + '"';	
		}
      }
...
function CustomSearchHighlight2(strSearchKeyWords)
{
	words = strSearchKeyWords.split(" "); 
	
	for (w=0;w<words.length;w++) {
		if( words[w] == '' ) continue;
		if(document.getElementById("tblCover")){
		highlightWord(document.getElementById("tblCover"),words[w]);
		searchhi_string = searchhi_string + ' "' + words[w] + '"';	
		}
      }
}
